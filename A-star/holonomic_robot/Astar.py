...
def aStar(startPoint, endPoint, resolution, xUnits, yUnits):
	...
	while not isEmpty(priorityQ):
		...
		for i in [-1, 0, 1]:
			for j in [-1, 0, 1]:
				...
				c2g = cost2go([currentNode[0]+i, currentNode[1]+j], endPoint)
				if i==0 or j==0:
					dist = gcost[currentNode[0], currentNode[1]] + 1 + c2g
				else:
					dist = gcost[currentNode[0], currentNode[1]] + 1.414 + c2g

				if not visitedFlag[currentNode[0]+i, currentNode[1]+j]:
					#	Mark this node as visited
					visitedFlag[currentNode[0]+i, currentNode[1]+j] = 1
					#	Update gcost
					gcost[currentNode[0]+i, currentNode[1]+j] = dist - c2g
					...
					if dist < gcost[currentNode[0]+i, currentNode[1]+j] + c2g:
